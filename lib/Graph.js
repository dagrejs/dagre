...
Graph.prototype.equals = function(other) {
  ...
  return thisUs.length === otherUs.length &&
         util.all(thisUs, function(x) { return other.hasNode(x) && self.node(x) === other.node(x); }) &&
         util.all(thisEs, function(x) { return other.hasEdge(x) && self.edge(x) === other.edge(x); });
};
