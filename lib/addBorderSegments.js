    ...
    function unwindStack(u) {
      while ((curr = stack[stack.length - 1]) !== u) {
        ...
        v = g.addNode(null, {
          ...
          width: 0,
          height: 0,
          rightBorderSegment: true
        });
        g.parent(v, curr);
        g.node(curr).rightBorderSegments[rank] = v;

        var prevNode = g.node(curr).rightBorderSegments[rank - 1];
        if (prevNode !== undefined) {
          g.addEdge(null, prevNode, v, {});
        }
      }
    }
    ...
    for (var i = 0, il = layer.length; i < il; ++i, ++nextOrder) {
      ...
      for (var j = 1, jl = path.length; j < jl; ++j) {
        ...
        v = g.addNode(null, {
          ...
          width: 0,
          height: 0,
          leftBorderSegment: true
        });
        g.parent(v, curr);
        g.node(curr).leftBorderSegments[rank] = v;

        var prevNode = g.node(curr).leftBorderSegments[rank - 1];
        if (prevNode !== undefined) {
          g.addEdge(null, prevNode, v, {});
        }
      }
