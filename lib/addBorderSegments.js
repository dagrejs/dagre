...
function addBorderSegments(g) {
  ...
  g.eachNode(function(u, value) {
    ...
    if (children.length) {
      ...
      for (i = min; i <= max; ++i) {
        leftSeg = g.addNode(leftIdGen(), { rank: i, minRank: i, maxRank: i, width: 0, height: 0, leftBorderSegment: true, dummy: true });
        rightSeg = g.addNode(rightIdGen(), { rank: i, minRank: i, maxRank: i, width: 0, height: 0, rightBorderSegment: true, dummy: true });

        g.parent(leftSeg, u);
        g.parent(rightSeg, u);

        if (prevLeftSeg !== undefined) {
          g.addEdge(leftIdGen(), prevLeftSeg, leftSeg, {});
          g.addEdge(rightIdGen(), prevRightSeg, rightSeg, {});
        }
