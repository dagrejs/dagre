...
module.exports = function() {
  ...
  function run(inputGraph) {
    ...
    try {
      ...
      // Assign min / max ranks to each subgraph.
      assignMinMaxRanks(g);
  ...
  function assignMinMaxRanks(g) {
    g.eachNode(function(u, value) {
      if (value.borderNodeTop !== undefined) {
        value.minRank = g.node(value.borderNodeTop).rank;
        value.maxRank = g.node(value.borderNodeBottom).rank;
      }
    });
  }
