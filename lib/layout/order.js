...
function sortLayer(g, movable, adjacent) {
  // We want to add nodes as needed to this graph. However, the copy contains
  // the original values, so we can update them through the copy.
  g = g.copy();
  ...
  movable.forEach(function(u) {
    ...
    bs[u] = (deg[u] > 0
        ...
        : 0);
    lists[u] = [u];
    nextOrder = Math.max(nextOrder, g.node(u).order);
  });
  ...
  while ((violated = findViolatedConstraint(cg, bs)) !== undefined) {
    ...
    var v = g.addNode(null, {order: ++nextOrder});
    cg.addNode(v);
