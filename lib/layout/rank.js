...
function combineRanks(g) {
  ...
  if (hasRankConstraint) {
    ...
    for (var r in ranks) {
      reduced.addNode(r.toString(), {originals: ranks[r]});
    }

    // Add edges for each rank crossing edge in the original graph.
    g.eachEdge(function(id, source, target, value) {
      var source_rank = g.node(source).rank;
      var target_rank = g.node(target).rank;
      if (source_rank != target_rank) {
        reduce.addEdge(null, source_rank, target_rank);
      }
    });
    ...
    function reverse(edges) {
      for (var e in edges) {
        ...
        reduced.addEdge(edges[e], target, source, {reversed: true});
      }
    };
    ...
    // Add edges from 'min' to any node without in-edges and from any node
    // without out-edges to 'max'.
    reduced.eachNode(function(n, value) {
      if (n != 'min' && reduced.inEdges(n).length == 0) {
        reduced.addEdge(null, 'min', n);
      }
      if (n != 'max' && reduced.outEdges(n).length == 0) {
        reduced.addEdge(null, n, 'max');
      }
    });
    return reduced;
  } else {
