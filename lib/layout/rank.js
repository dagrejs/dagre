...
function setCutValue(graph, tree, child) {
  ...
  var outEdges = tree.outEdges(child);
  var u, v;
  var uEdge, vEdge;
  ...
  var E = 0;    // Edges from child to u's subtree.
  var F = 0;    // Edges to child from u's subtree.
  var G = 0;    // Edges from child to nodes outside of child's subtree.
  var H = 0;    // Edges from nodes outside of child's subtree to child.
  var I = 0;    // Edges from child to v's subtree.
  var J = 0;    // Edges to child from u's subtree.
  var outEdges = graph.outEdges(child);
  for (var oe in outEdges) {
    var succ = graph.target(outEdges[oe]);
    if (u && inSubtree(tree, succ, u)) {
      E++;
    }
    if (!inSubtree(tree, succ, child)) {
      console.log('    ' + succ + ' is not in ' + child + '\'s subtree');
      G++;
    }
    if (v && inSubtree(tree, succ, v)) {
      I++;
    }
  }
  ...
  for (var ie in inEdges) {
    ...
    if (u && inSubtree(tree, pred, u)) {
      F++;
    }
    if (!inSubtree(tree, pred, child)) {
      H++;
    }
    if (v && inSubtree(tree, pred, v)) {
      J++;
    }
  }

  console.log("E: " + E + " F: " + F + " G: " + G + " H: " + H + " I: " + I + " J: " + J);

  // Contributions depend on the alignment of the parent -> child edge
  // and the child -> u or v edges.
  var ucv = 0;
  if (u) {
    ucv = tree.edge(uEdge).cutValue;
    if (tree.source(uEdge) != graph.source(uEdge)) {
      ucv = -ucv;
    }
  }
  ...
  var vcv = 0;
  if (v) {
    vcv = tree.edge(vEdge).cutValue;
    if (tree.source(vEdge) != graph.source(vEdge)) {
      vcv = -vcv;
    }
  }

  if (tree.source(parentEdge) == graph.source(parentEdge)) {
    cutValue += ucv - E + F - G + H + vcv - I + J;
  } else {
    cutValue -= ucv - E + F - G + H + vcv - I + J;
  }
