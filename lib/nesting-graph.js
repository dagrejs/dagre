...
function run(g) {
  ...
  _.each(g.getChildren(), function(child) { dfs(g, root, nodeSep, weight, child); });

  // Save the multiplier for node layers for later removal of empty border
  // layers.
  g.getGraph().nodeRankFactor = nodeSep;
}

function dfs(g, root, nodeSep, weight, v) {
  ...
  _.each(children, function(child) {
    dfs(g, root, nodeSep, weight, child);
    ...
    g.setEdge(top, childTop, { weight: thisWeight, minlen: 1, nestingEdge: true });
    g.setEdge(childBottom, bottom, { weight: thisWeight, minlen: 1, nestingEdge: true });
  });
