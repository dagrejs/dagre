...
function expandSelfLoops(g) {
  ...
  g.eachEdge(function(e, u, v, a) {
    if (u === v) {
      var x = addDummyNode(g, e, u, v, a, 0, false, 'selfLoopDummy', idGen),
          y = addDummyNode(g, e, u, v, a, 1, true,  'selfLoopDummy', idGen),
          z = addDummyNode(g, e, u, v, a, 2, false, 'selfLoopDummy', idGen),
          parent = g.parent(u);
      g.parent(x, parent);
      g.parent(y, parent);
      g.parent(z, parent);
      // Note: we add the points arrays because it is possible that dummy nodes
      // will be added between these edges and the normalize undo code tries to
      // put the positions of those dummy nodes in the points array for the
      // original edge. We actually don't case about the values that are
      // returned - we'll just use the position of the dummy nodes we added
      // instead.
      g.addEdge(idGen(), x, u, {minLen: 1, width: 0, height: 0, points: []});
      g.addEdge(idGen(), x, y, {minLen: 1, width: 0, height: 0, points: []});
      g.addEdge(idGen(), u, z, {minLen: 1, width: 0, height: 0, points: []});
      g.addEdge(idGen(), y, z, {minLen: 1, width: 0, height: 0, points: []});
      g.delEdge(e);
    }
  });
}

function restoreSelfLoops(g) {
  g.eachNode(function(u, a) {
    if (a.selfLoopDummy) {
      ...
      edge.attrs.points[a.index] = { x: a.x, y: a.y, ul: a.ul, ur: a.ur, dl: a.dl, dr: a.dr };
      g.delNode(u);
    }
  });
}
  ...
  g.eachEdge(function(e, u, v, a) {
    if (u === v) {
      var origEdge = a.originalEdge,
          // TODO: special handling for sideways edges
          dummy = addDummyNode(g, origEdge.e, origEdge.u, origEdge.v, origEdge.value, 0, true, 'dummySegment', idGen);
...
function addDummyNode(g, e, u, v, a, index, isLabel, type, idGen) {
  var attrs = {
